...
<?php if(is_array($categoryq)): $i = 0; $__LIST__ = $categoryq;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cate): $mod = ($i % 2 );++$i;?><div class="item">
   ...
   <section>
      ...
      <dd><?php if(is_array($category)): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a href="<?php echo U('Tuan/category?id='.$vo['id']);?>"><?php echo ($vo["title"]); ?>(<?php echo (get_tuan_count($vo["id"])); ?>)</a><?php endforeach; endif; else: echo "" ;endif; ?> </dd></dl> 
       </div>   
       
       <ul class="ml_content_top"><li <?php if(($order) == "1"): ?>class="active"<?php else: endif; ?>><a rel="nofollow" url="<?php echo U('tuan/index','order=1'.'&sort='.$see);?>" href="<?php echo U('tuan/index','category='.$category['name'].'&order=1'.'&sort='.$see);?>" class="<?php echo ($value); ?>">热度<i></i></a></li><li <?php if(($order) == "2"): ?>class="active"<?php else: endif; ?>><a rel="nofollow" href="<?php echo U('tuan/index','category='.$category['name'].'&order=2'.'&sort='.$see);?>"  class="<?php echo ($value); ?>">最新<i></i></a></li><li <?php if(($order) == "3"): ?>class="active"<?php else: endif; ?>><a rel="nofollow" href="<?php echo U('tuan/index','category='.$category['name'].'&order=3'.'&sort='.$see);?>"  class="<?php echo ($value); ?>">价格<i></i></a></li><li <?php if(($order) == "4"): ?>class="active"<?php else: endif; ?>><a rel="nofollow" href="<?php echo U('tuan/index','category='.$category['name'].'&order=4'.'&sort='.$see);?>"  class="<?php echo ($value); ?>">销量<i></i></a></li></ul> 
               ...
               <a href="<?php echo U('Article/detail?id='.$vo['goodid']);?>"><img src="<?php echo (get_cover(get_cover_id($vo["goodid"]),'path')); ?>"> </a> 
               <p class="title"><?php echo ($vo["title"]); ?></p>
			    <p><span class="tuan"><strong>￥<?php echo (get_good_tuanprice($vo["goodid"])); ?></strong>/<strong class="yuan">￥<?php echo (get_good_price($vo["goodid"])); ?></strong></span><span class="qiang"><a>去抢购</a></span></p>
				  <p class="time">剩余时间：<?php date_default_timezone_set("Asia/Shanghai"); $time1 = strtotime('2015-3-17 11:11:11'); $time2 =time(); $second = $time1 - $time2; $day = floor($second/(3600*24)); $second = $second%(3600*24); $hour = floor($second/3600); $second = $second%3600; $minute = floor($second/60); $second = $second%60; echo $day.'天'.$hour.'小时'.$minute.'分'.$second.'秒'; ?> </p>
